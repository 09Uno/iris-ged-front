...
export class DashboardComponent implements OnInit {
  ...
  validateSearch(): void {
    this.uiControllers.validSearch = 
      this.searchFilters.textoPesquisa.length > 0 ||
      this.searchFilters.numeroDocumento.length > 0 ||
      this.searchFilters.assunto.length > 0 ||
      this.searchFilters.orgaoGerador.length > 0 ||
      this.searchFilters.autor.length > 0;
  }
  ...
  viewDocument(document?: SearchDocumentItem): void {
    if (document) {
      ...
      // Navegar para o document-manager com o protocolo do documento
      const protocol = document.protocolNumber || document.generatedProtocol;
      if (protocol) {
        console.log('Navegando para document-manager com protocolo e documento:', protocol, document.id);
        // Navegar com query parameters para protocolo e documento
        ...
        console.warn('Documento não possui protocolo para navegação');
        // Navegar sem protocolo mas com documento
        this.router.navigate(['/documentos'], {
